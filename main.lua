...
function BookVault:loadSettings()
    ...
    if type(self.settings.data.visible_statuses) ~= "table" then
        self.settings.data.visible_statuses = {
            all = true, reading = true, abandoned = true, complete = true, new = true,
        }
    end
...
function BookVault:verifyPassword(password)
    return self:hasPassword()
        and self:hashPassword(password, self.settings.data.password_salt) == self.settings.data.password_hash
end
        ...
        buttons = {{
            ...
            { text = _("OK"), is_enter_default = true, callback = function()
                ...
                if self:verifyPassword(password) then
                    safe(function() callback(true) end)
                else
                    ...
                    second = InputDialog:new{
                        ...
                        buttons = {{
                            ...
                            { text = _("Salvar"), is_enter_default = true, callback = function()
                                ...
                                if on_saved then safe(on_saved) end
                            end },
                        }},
                    }
...
function BookVault:scanBooks(include_private)
    ...
    local result, visited, pending = {}, {}, { root }
    local index = 1
                ...
                for name in iter, obj do
                    if name ~= "." and name ~= ".." then
                        ...
                        elseif attr and attr.mode == "file" and (include_private or not self:isPrivate(path)) then
                            ...
                            if pok and provider then
                                result[#result + 1] = { path = path, filepath = path, text = name, attr = attr, is_file = true }
                            end
                        end
                    end
                end
...
function BookVault:getStatusLabel(status)
    for _, s in ipairs(STATUS) do
        if s.key == status then return s.label end
    end
    ...
    for name, coll in pairs(ReadCollection.coll) do
        if type(name) == "string" and type(coll) == "table" then
            ...
            collections[#collections + 1] = {
                name = name, order = tonumber(settings.order) or 999999, count = 0,
            }
...
function BookVault:isCollectionVisible(name)
    self:loadSettings()
    return self.settings.data.visible_collections[name] ~= false
end
...
function BookVault:collectionItems(collection_name, include_private)
    ...
    for file, item in pairs(coll) do
        ...
        if path and contains(root, path) and (include_private or not self:isPrivate(path)) and lfs.attributes(path, "mode") == "file" then
            items[#items + 1] = {
                path = path, filepath = path, text = item.text or path:match("[^/]+$"),
                attr = lfs.attributes(path), is_file = true,
            }
        end
    end
...
function BookVault:sortBookVaultItems(menu, mode)
    ...
    local items = menu._bookvault_source_items or menu.item_table or {}
    local sorted = {}
    ...
    menu._bookvault_source_items = sorted
    menu.item_table = sorted
    menu.page = 1
    if menu._bookvault_sort_dialog then
        UIManager:close(menu._bookvault_sort_dialog)
        menu._bookvault_sort_dialog = nil
    end
    menu:updateItems()
end

-- Loads KOReader's bundled CoverBrowser internals only after BookVault is opened.
-- No global classes are changed. The methods are attached only to this BookList instance.
function BookVault:loadVisualModules()
    local old_path = package.path
    local candidates = {
        "plugins/coverbrowser.koplugin/?.lua",
        "./plugins/coverbrowser.koplugin/?.lua",
    }
...
function BookVault:ensureCatIcon()
    ...
    if lfs.attributes(icon_dir, "mode") ~= "directory" then
        local ok = pcall(lfs.mkdir, icon_dir)
        if not ok and lfs.attributes(icon_dir, "mode") ~= "directory" then return false end
    end
...
function BookVault:prepareVisualMenu(menu, source_items)
    ...
    menu.title_bar_left_icon = self:ensureCatIcon() and "bookvault-cat" or "appbar.search"

    function menu:onLeftButtonTap()
        local dialog
        dialog = InputDialog:new{
            title = _("Buscar na biblioteca"), input = "", input_type = "text",
            buttons = {{
                { text = _("Cancelar"), callback = function() UIManager:close(dialog) end },
                { text = _("Buscar"), is_enter_default = true, callback = function()
                    local query = (dialog:getInputText() or ""):lower():gsub("^%s+", ""):gsub("%s+$", "")
                    UIManager:close(dialog)
                    if query == "" then
                        self.item_table = self._bookvault_source_items
                    else
                        local filtered = {}
                        for _, item in ipairs(self._bookvault_source_items) do
                            if (item.text or ""):lower():find(query, 1, true) then filtered[#filtered + 1] = item end
                        end
                        self.item_table = filtered
                    end
                    self.page = 1
                    self:updateItems()
                end },
    ...
    function menu:onLeftButtonHold()
        local buttons = {
            {{ text = _("Título"), callback = function() self:sortBookVaultItems("title") end }},
            {{ text = _("Mais recentes"), callback = function() self:sortBookVaultItems("recent") end }},
            {{ text = _("Modificados recentemente"), callback = function() self:sortBookVaultItems("modified") end }},
            {{ text = _("Tamanho"), callback = function() self:sortBookVaultItems("size") end }},
            {{ text = _("Cancelar"), callback = function() UIManager:close(self._bookvault_sort_dialog) end }},
        }
        self._bookvault_sort_dialog = ButtonDialog:new{
            title = _("Ordenar livros"), title_align = "center", buttons = buttons,
        }
        UIManager:show(self._bookvault_sort_dialog)
    end

    local BookInfoManager, CoverMenu, MosaicMenu = self:loadVisualModules()
    if not BookInfoManager then return false end

    -- These are the same dimensions used by KOReader CoverBrowser's mosaic mode.
    ...
    -- MosaicMenu uses this callback to obtain reading information from the
    -- bundled CoverBrowser database and to extract/cache the actual cover.
    menu.getBookInfo = function(_, filepath)
...
function BookVault:makeBookMenu(name, title, items)
...
function BookVault:showCollection(collection_name)
    safe(function()
        ...
        local menu = self:makeBookMenu(
            "bookvault_collection_" .. collection_name,
            _("BookVault") .. " · " .. collection_name,
            items
        )
        UIManager:show(menu)
        menu:updateItems()
...
function BookVault:showCollectionChooser()
    ...
    for _, collection in ipairs(collections) do
        if self:isCollectionVisible(collection.name) then
            buttons[#buttons + 1] = {{ text = collection.name, callback = function()
                ...
                self:showCollection(collection.name)
            end }}
        end
    end
    if #buttons == 0 then
    ...
    self.collection_dialog = ButtonDialog:new{ title = _("Coleções"), title_align = "center", buttons = buttons }
...
function BookVault:showCollectionVisibilityChooser()
    ...
    local function rebuild()
        ...
        self.collection_visibility_dialog = ButtonDialog:new{ title = _("Coleções exibidas"), title_align = "center", buttons = buttons }
        UIManager:show(self.collection_visibility_dialog)
    end
    rebuild()
end
...
function BookVault:showLibrary(status, include_private)
    safe(function()
        ...
        for _, item in ipairs(self:scanBooks(include_private)) do
            if status == "all" or BookList.getBookStatus(item.path) == status then items[#items + 1] = item end
        end
        local menu = self:makeBookMenu(
            "bookvault_library",
            _("BookVault") .. " · " .. self:getStatusLabel(status),
            items
        )
        UIManager:show(menu)
        menu:updateItems()
...
function BookVault:showStatusChooser()
    ...
    if #buttons == 0 then
        ...
        self:saveSettings()
        buttons = {{{ text = STATUS[1].label, callback = function()
            UIManager:close(self.status_dialog)
            self:showLibrary("all", self.unlocked)
        end }}}
    end
    self.status_dialog = ButtonDialog:new{ title = _("BookVault"), title_align = "center", buttons = buttons }
...
function BookVault:showStatusVisibilityChooser()
    ...
    local function rebuild()
        ...
        self.status_visibility_dialog = ButtonDialog:new{ title = _("Categorias exibidas"), title_align = "center", buttons = buttons }
...
function BookVault:chooseManagedPath(private)
    ...
    UIManager:show(PathChooser:new{
        ...
        onConfirm = function(path)
            ...
            addUnique(list, path)
            self:saveSettings()
            UIManager:show(InfoMessage:new{ text = private and _("Pasta tornada privada.") or _("Pasta protegida.") })
        end,
    })
end
...
function BookVault:listManagedPaths(private)
    ...
    local function showList()
        ...
        for i, path in ipairs(list) do
            buttons[#buttons + 1] = {{ text = path, callback = function()
                table.remove(list, i)
                self:saveSettings()
        ...
        self.path_dialog = ButtonDialog:new{ title = private and _("Conteúdo privado") or _("Pastas protegidas"), buttons = buttons }
        UIManager:show(self.path_dialog)
    end
    if self:hasPassword() then
        self:askPassword(function(ok) if ok then showList() end end)
    else
        showList()
    end
end
...
function BookVault:togglePrivate()
    ...
    if not self:hasPassword() then
        ...
        self:askPassword(function(ok)
            if ok then self.unlocked = true; self:showStatusChooser() end
...
function BookVault:addToMainMenu(menu_items)
    menu_items.bookvault = {
        text = _("BookVault"), sorting_hint = "more_tools", sub_item_table = {
