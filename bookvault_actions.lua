...
local function copyItems(items) local r={}; for i,v in ipairs(items or {}) do r[i]=v end; return r end
...
function M.install(BV)
    ...
    function BV:showCollectionsForBook(item,menu)
        ...
        for _,name in ipairs(names) do
            local checked=ReadCollection:isFileInCollection(item.path,name)
            buttons[#buttons+1]={{text=(checked and "☑ " or "☐ ")..name,callback=function()
                local map={}; for _,n in ipairs(names) do map[n]=ReadCollection:isFileInCollection(item.path,n) end
                map[name]=not checked; ReadCollection:addRemoveItemMultiple(item.path,map); ReadCollection:write()
                UIManager:close(self._bookvault_collection_dialog); self._bookvault_collection_dialog=nil; self:showCollectionsForBook(item,menu)
            end}}
        end
        buttons[#buttons+1]={{text=_("Nova coleção"),callback=function()
            local d; d=InputDialog:new{title=_("Nova coleção"),input="",buttons={{text=_("Cancelar"),callback=function() UIManager:close(d) end},{text=_("Criar"),is_enter_default=true,callback=function()
                local name=d:getInputText(); UIManager:close(d)
                if name and name~="" and not ReadCollection.coll[name] then ReadCollection:addCollection(name); ReadCollection:addItem(item.path,name); ReadCollection:write() end
                self:showCollectionsForBook(item,menu)
            end}}}; UIManager:show(d); pcall(d.onShowKeyboard,d)
        end}}
    ...
    function BV:renameBook(item,menu)
        local d; d=InputDialog:new{title=_("Renomear livro"),input=basename(item.path),buttons={{text=_("Cancelar"),callback=function() UIManager:close(d) end},{text=_("Salvar"),is_enter_default=true,callback=function()
            local name=d:getInputText(); UIManager:close(d); if not name or name=="" then return end
            local dir=item.path:match("^(.*)/[^/]+$") or "."; local dest=dir.."/"..name
            if lfs.attributes(dest) then UIManager:show(InfoMessage:new{text=_("Já existe um arquivo com esse nome.")}); return end
            if os.rename(item.path,dest) then
                ReadCollection:updateItem(item.path,dest); pcall(function() require("readhistory"):updateItem(item.path,dest) end)
                item.path=dest; item.filepath=dest; item.text=name; if menu then menu:updateItems() end
            else UIManager:show(InfoMessage:new{text=_("Não foi possível renomear o arquivo.")}) end
        end}}; UIManager:show(d); pcall(d.onShowKeyboard,d)
    end

    function BV:deleteBooks(paths,menu)
        local files={}; for path in pairs(paths) do files[path]=true end
        UIManager:show(ConfirmBox:new{text=T(N_("Excluir %1 livro?","Excluir %1 livros?",count(files)),count(files)),ok_text=_("Excluir"),ok_callback=function()
            local failed=0
            for path in pairs(files) do if not os.remove(path) then failed=failed+1 else ReadCollection:removeItem(path,nil,true) end end
            ReadCollection:write(); pcall(function() require("readhistory"):clearMissing() end)
            if failed>0 then UIManager:show(InfoMessage:new{text=T(_("%1 arquivo(s) não puderam ser excluídos."),failed)}) end
            if menu then self:leaveSelection(menu); menu._bookvault_source_items=self:scanBooks(self:privacyIncludePrivate()); menu.item_table=menu._bookvault_source_items; menu:updateItems() end
        end})
    end

    function BV:copyOrMoveBook(item,menu,move)
        UIManager:show(PathChooser:new{select_directory=true,select_file=false,onConfirm=function(dir)
            if not dir then return end
            local dest=real(dir).."/"..basename(item.path)
            if lfs.attributes(dest) then UIManager:show(InfoMessage:new{text=_("Já existe um arquivo com esse nome no destino.")}); return end
            local ok
            if move then ok=os.rename(item.path,dest); if ok then ReadCollection:updateItem(item.path,dest); pcall(function() require("readhistory"):updateItem(item.path,dest) end) end else ok=ffiUtil.copyFile(item.path,dest) end
            if not ok then UIManager:show(InfoMessage:new{text=move and _("Não foi possível mover o arquivo.") or _("Não foi possível copiar o arquivo.")}) end
            if menu then menu:updateItems() end
        end})
    end

    function BV:showPluginActions(menu,item)
        local ui=self.ui; local rows={}
        for name,mod in pairs(ui or {}) do if type(mod)=="table" and name~="bookvault" and type(mod.addFileDialogButtons)=="function" then rows[#rows+1]=name end end
        if #rows==0 then UIManager:show(InfoMessage:new{text=_("Nenhuma ação de plugin compatível foi encontrada.")}); return end
        table.sort(rows); local buttons={}; for _,name in ipairs(rows) do buttons[#buttons+1]={{text=name,enabled=false}} end
        buttons[#buttons+1]={{text=_("Os plugins compatíveis aparecem aqui quando disponibilizam ações ao File Browser."),enabled=false}}
        buttons[#buttons+1]={{text=_("Concluído"),callback=function() UIManager:close(self._bookvault_plugins_dialog) end}}
        self._bookvault_plugins_dialog=ButtonDialog:new{title=_("Ações de plugins"),buttons=buttons}; UIManager:show(self._bookvault_plugins_dialog)
    end
    ...
    function BV:showBookActions(menu,item)
        ...
        local buttons={
            ...
            {{text=_("Buscar capa no Google Imagens"),callback=function() UIManager:close(self._bookvault_action_dialog); self:searchGoogleImagesForCover(item.path) end}},
            {{text=_("Ações de plugins"),callback=function() UIManager:close(self._bookvault_action_dialog); self:showPluginActions(menu,item) end}},
    ...
    function BV:searchGoogleImagesForCover(file)
        ...
        local sink={}; socketutil:set_timeout(10,20)
        local ok=pcall(function() return socket.skip(1,http.request{url=url,headers={["User-Agent"]="Mozilla/5.0",["Accept-Encoding"]="identity"},sink=ltn12.sink.table(sink)}) end)
        socketutil:reset_timeout()
        local html=ok and table.concat(sink) or ""
        local found={}; local seen={}
        for u in html:gmatch('https?://[^"\\<> ]+') do
            u=u:gsub("\\u003d","="):gsub("\\u0026","&")
            if (u:find("gstatic.com/images") or u:match("%.[Jj][Pp][Gg]") or u:match("%.[Pp][Nn][Gg]") or u:match("%.[Ww][Ee][Bb][Pp]")) and not seen[u] then
                seen[u]=true; found[#found+1]=u; if #found>=6 then break end
            end
        end
        ...
        for i,u in ipairs(found) do
            local idx=i
            buttons[#buttons+1]={{text=T(_("Usar resultado %1"),idx),callback=function()
                local cache=DataStorage:getDataDir().."/bookvault/covers"; pcall(lfs.mkdir,DataStorage:getDataDir().."/bookvault"); pcall(lfs.mkdir,cache)
                local out=cache.."/cover_"..tostring(os.time()).."_"..idx..".jpg"; local f=io.open(out,"wb"); if not f then return end
                socketutil:set_timeout(10,20); local ok2=pcall(function() return socket.skip(1,http.request{url=u,sink=ltn12.sink.file(f),headers={["User-Agent"]="Mozilla/5.0"}}) end); socketutil:reset_timeout(); pcall(f.close,f)
                if not ok2 or lfs.attributes(out,"mode")~="file" then UIManager:show(InfoMessage:new{text=_("Não foi possível baixar esta capa.")}); return end
                local bi=self.ui and self.ui.bookinfo
                if bi and bi.setCustomCoverFromImage then bi:setCustomCoverFromImage(file,out); UIManager:close(self._google_cover_dialog) else UIManager:show(InfoMessage:new{text=_("A edição de capa nativa não está disponível nesta tela.")}) end
            end}}
        end
        buttons[#buttons+1]={{text=_("Cancelar"),callback=function() UIManager:close(self._google_cover_dialog) end}}
        self._google_cover_dialog=ButtonDialog:new{title=_("Google Imagens · capas"),buttons=buttons}; UIManager:show(self._google_cover_dialog)
    end
end
return M
