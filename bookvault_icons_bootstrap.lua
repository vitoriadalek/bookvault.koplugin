...
local icons={ ["bookvault-search.png"]="bookvault-search.png", ["bookvault-sort.png"]="bookvault-sort.png", ["bookvault-sort-cat.png"]="bookvault-sort-cat.png", ["bookvault-cat.png"]="bookvault-cat.png", ["bookvault-moon.png"]="bookvault-moon.png" }
if plugin_dir and lfs.attributes(icon_dir,"mode")=="directory" then
    for target_name,source_name in pairs(icons) do
        local stem=target_name:gsub("%.png$",""); pcall(os.remove,icon_dir.."/"..stem..".svg")
        local input=io.open(plugin_dir.."/icons/"..source_name,"rb")
        if input then local data=input:read("*a"); input:close(); if data and #data>0 then local output=io.open(icon_dir.."/"..target_name,"wb"); if output then output:write(data); output:close() end end end
    end
end

-- Compatibility bridge: only the BookVault class is decorated; KOReader's
-- FileManager/FileChooser classes are never replaced.
local gettext=require("gettext")
_G.N_=gettext.ngettext
...
WidgetContainer.extend=function(base,props,...)
    ...
    if not installed and props and props.name=="bookvault" then
        ...
        else pcall(require("logger").err,"BookVault action layer unavailable",actions) end
